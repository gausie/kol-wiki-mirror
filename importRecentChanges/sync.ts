...
async function getChanges(): Promise<RecentChanges> {
  const response = await fetch(
    "https://kol.coldfront.net/thekolwiki/api.php?action=query&list=recentchanges&rcprop=title|ids|sizes|timestamp|user|userid|comment&rclimit=100&format=json",
  );
...
async function getMirrorChanges(): Promise<RecentChanges> {
  const response = await fetch(
    "https://kol.ar.gy/api.php?action=query&list=recentchanges&rcprop=title|ids|sizes|timestamp|user|userid&rcnamespace=0|1&rclimit=100&format=json",
  );
  return await response.json();
}
...
async function findPageInfo(c: RecentChange): Promise<ExistingPage> {
  const response = await fetch(
    `https://kol.ar.gy/api.php?action=query&titles=${c.title}&prop=revisions&format=json`,
  );
  const result = await response.json();
  return Object.values(result.query.pages as ExistingPage[]).filter(
    (p) => p.ns === c.ns,
  )[0];
}
...
async function exportRevision(c: RecentChange) {
  const response = await fetch(
    `https://kol.coldfront.net/thekolwiki/api.php?action=query&format=json&export=1&revids=${c.revid}`,
  );
  ...
  if (existingPage) {
    exported = exported.replace(
      `<id>${c.pageid}</id>`,
      `<id>${existingPage.pageid}</id>`,
    );
  }

  if (c.ns > 99)
    exported = exported.replace(`<ns>${c.ns}</ns>`, `<ns>${c.ns + 2900}</ns>`);
...
async function getImageUrl(filePage: string) {
  const response = await fetch(
    `https://kol.coldfront.net/thekolwiki/api.php?action=query&format=json&prop=imageinfo&iiprop=url&titles=${filePage}`,
  );
  const result = (await response.json()) as {
    query: {
      pages: {
        [pageid: number]: { pageid: number; imageinfo: { 0: { url: string } } };
      };
    };
  };
...
async function applyChanges(changes: RecentChange[]) {
  for (let i = changes.length; i > 0; i--) {
    ...
    if (c.type === "log" && c.title.startsWith("File:")) {
      ...
      const res =
        await execa`docker exec -i kol-wiki-mediawiki-1 /var/www/html/maintenance/run importImages ${[
      ...
      await execa({
        input: c.title,
      })`docker exec -i kol-wiki-mediawiki-1 /var/www/html/maintenance/run purgeList`;
      console.log(`${c.type} ${c.title}: ${res.stdout.split("\n")[2]}`);
    } else {
      const xml = await exportRevision(c);
      const res = await execa({
        input: xml,
      })`docker exec -i kol-wiki-mediawiki-1 /var/www/html/maintenance/run importDump`;
      console.log(`${c.type} ${c.title}: ${res.stdout.split("\n")[0]}`);
    }
  }

  const from = format(
    new Date(changes[changes.length - 1].timestamp).getTime() - 1000,
    "yyyyMMddHHmmss",
  );
  const to = format(
    new Date(changes[0].timestamp).getTime() + 1000,
    "yyyyMMddHHmmss",
  );
...
async function main() {
  ...
  const indexOfLatestMirrored = changes.query.recentchanges.findIndex((c) =>
    compareChanges(c, latestMirrored),
  );

  const unmirrored = changes.query.recentchanges.slice(
    0,
    indexOfLatestMirrored,
  );
